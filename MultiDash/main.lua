...
local function configure(w)
    ...
    addNumber("Cells", 0, 6, function() return w.cellCount or 0 end, function(v) w.cellCount = clamp(tonumber(v) or 0, 0, 6) end, 0)
    ...
    addChoice("Arm switch", armSwitchChoices,
    ...
    form.addSourceField(line, nil, function() return w.batterySource end, function(v) w.batterySource = v end)
    line = form.addLine("Link quality")
    form.addSourceField(line, nil, function() return w.rssiSource end, function(v) w.rssiSource = v end)
    line = form.addLine("Telemetry 1")
    form.addSourceField(line, nil, function() return w.field1Source end, function(v) w.field1Source = v end)
    line = form.addLine("Telemetry 2")
    form.addSourceField(line, nil, function() return w.field2Source end, function(v) w.field2Source = v end)
    line = form.addLine("Telemetry 3")
    form.addSourceField(line, nil, function() return w.field3Source end, function(v) w.field3Source = v end)
    line = form.addLine("Telemetry 4")
    form.addSourceField(line, nil, function() return w.field4Source end, function(v) w.field4Source = v end)
    line = form.addLine("Timer")
    form.addSourceField(line, nil, function() return w.timerSource end, function(v) w.timerSource = v end)
    line = form.addLine("Current")
    form.addSourceField(line, nil, function() return w.currentSource end, function(v) w.currentSource = v end)
    line = form.addLine("RPM")
    form.addSourceField(line, nil, function() return w.rpmSource end, function(v) w.rpmSource = v end)
    ...
    form.addLine("In-Flight Screen Defines")
    ...
    form.addSourceField(line, nil, function() return w.inFlight1Source end, function(v) w.inFlight1Source = v end)
    line = form.addLine("In-flight stat 2")
    form.addSourceField(line, nil, function() return w.inFlight2Source end, function(v) w.inFlight2Source = v end)
    line = form.addLine("In-flight stat 3")
    form.addSourceField(line, nil, function() return w.inFlight3Source end, function(v) w.inFlight3Source = v end)
    line = form.addLine("In-flight stat 4")
    form.addSourceField(line, nil, function() return w.inFlight4Source end, function(v) w.inFlight4Source = v end)
end
...
local function statusText(face)
...
local function cellsFor(w, batt)
    ...
    local autoCells = math.max(1, math.min(6, math.floor(batt / maxCell + 0.5)))
...
local function drawPostFlight(w, c, scale, scrW, scrH)
    ...
    setFontSize("large", scale)
    local summaryTxt = "Flight Summary"
    local summaryW = getTextW(summaryTxt) or 0
    local summaryX = margin + math.floor((width - summaryW) / 2)
    drawBold(summaryTxt, summaryX, titleY, c.muted)
...
local function drawBatteryDial(w, c, scale, scrW, scrH, batt, ratio, battCol)
    ...
    lcd.drawText(cx - math.floor((getTextW("Batt") or 0) / 2), cy + px(26, scale, 12, 34), "Batt")
...
local function drawNoTelemetry(scrW, scrH)
...
local function drawInFlight(w, c, scale, scrW, scrH)
    ...
    local battBottom = scrH - timerH - px(72, scale, 42, 82)
    local mainLeft = margin
    local mainRight = rightX - px(18, scale, 8, 24)
    if mainRight < mainLeft + px(220, scale, 150, 260) then mainRight = scrW - margin end
    local mainW = mainRight - mainLeft
    local battW = math.floor(mainW * 0.78)
    battW = clamp(battW, px(230, scale, 160, 300), math.floor(mainW * 0.90))
    local battH = math.floor(battW * 0.38)
    battH = clamp(battH, px(76, scale, 48, 92), px(158, scale, 92, 176))
    local bx = mainLeft + math.floor((mainW - battW) / 2)
    ...
    local segW = math.floor((interiorW - (5 - 1) * space) / 5)
    local segH = battH - 6
    for i = 1, 5 do
    ...
    if w.currentSource then
        setFontSize("large", scale)
        lcd.color(c.secondary)
        local currText = "Curr " .. formatValue(getVal(w.currentSource)) .. "A"
        lcd.drawText(battCenter - math.floor((getTextW(currText) or 0) / 2), textY + px(46, scale, 26, 54), currText)
    end
    ...
    local ty = scrH - px(64, scale, 34, 74)
    local b = px(1, scale, 1, 2)
...
local function paint(w)
    ...
    local imageW = px(245, visualScale, 58, math.floor(scrW * 0.39))
    local imageH = px(147, visualScale, 42, math.floor(scrH * 0.37))
    ...
    if w.flightActive and tonumber(w.inFlightScreen) ~= 2 then
        ...
        if not telemetryOk then drawNoTelemetry(scrW, scrH) end
        return
    end
    ...
    if w.imageFile and w.imageFile ~= "" then
        if not w.selectedBmp or w.selectedFile ~= w.imageFile then
            ...
            if lcd.loadBitmap then
                w.selectedBmp = lcd.loadBitmap(w.imageFile)
            end
        end
    ...
    if not drawn and not w.iconLoaded and lcd.loadBitmap then
        w.iconLoaded = true
        w.iconBmp = lcd.loadBitmap("SCRIPTS:/MultiDash/MultiDash.png")
    end
    ...
    local timerY = scrH - px(50, scale, 24, 60)
    local timerBold = px(1, scale, 1, 2)
...
local function wakeup(w)
    if w then
        updateFlight(w)
        local now = os.clock()
        if not w.lastSave or now - w.lastSave > 2 then
            write(w)
            w.lastSave = now
        end
    end
